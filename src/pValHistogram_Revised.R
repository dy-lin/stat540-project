...
#pValues Revised 
#------------------coef intercept:cancer_type
pvalues6 <- topTable(fit5, coef = 1:2 , number = Inf) 
pvalHistogram6 <- pvalues6 %>% 
  ggplot (aes(P.Value)) + geom_histogram() +  labs(title = "cancer_type*age model pValue Histogram for intercept:cancerType coefficient, coef=1:2")
pvalHistogram6

#----------------coef cancer_type 
pvalues7 <- topTable(fit5, coef = 2 , number = Inf) 
pvalHistogram7 <- pvalues7 %>% 
  ggplot (aes(P.Value)) + geom_histogram() + labs(title = "cancer_type*age model pValue Histogram for cancerType coefficient, coef=2")
pvalHistogram7

#----------------coef age 
pvalues8 <- topTable(fit5, coef = 3, number = Inf) 
pvalHistogram8 <- pvalues8 %>% 
  ggplot (aes(P.Value)) + geom_histogram() + labs(title = "cancer_type*age model pValue Histogram for age coefficient, coef=3")
pvalHistogram8
